...
def run(label):
    ...
    write_summary_md(stats, title, out_dir)
    print(f"outputs -> {out_dir}")


def write_summary_md(s, title, out_dir):
    """Human-readable summary.md regenerated from the stats of each run."""
    yrs = f"{s['years'][0]}-{s['years'][-1]}"
    k, r = s["k"], s["r"]
    pair = np.array(s["hub_size_signal_share"]["by_pair"])
    exp_share = s["hub_size_signal_share"]["export"]
    imp_share = s["hub_size_signal_share"]["import"]
    R = np.array([s["R"][c] for c in s["countries"]])
    C = np.array([s["C"][c] for c in s["countries"]])

    def hub_rows(L, n):
        rows = []
        for j in range(n):
            lead = np.argsort(-np.abs(L[:, j]))[:8]
            rows.append(", ".join(f"{s['countries'][i]} {L[i, j]:+.2f}" for i in lead))
        return rows
    ...
    lines = [
        ...
        f"- **Fit:** uncentered R^2 {s['r2_total']:.3f} (rank-1 baseline {s['r2_rank1']:.3f}).",
        "",
        f"## Top traders, {yrs} ($B)",
        "",
    ...
    ex = list(s["top10"]["exporters_usd_bn"].items())
    im = list(s["top10"]["importers_usd_bn"].items())
    for n, ((ec, ev), (ic, iv)) in enumerate(zip(ex, im), 1):
        lines.append(f"| {n} | {ec} | {ev:.1f} | {ic} | {iv:.1f} |")
    lines += [
        "",
        "## Hub structure",
        "",
        f"Export-hub sizes: {', '.join(f'hub {j+1} {v:.0%}' for j, v in enumerate(exp_share))}.",
        f"Import-hub sizes: {', '.join(f'hub {j+1} {v:.0%}' for j, v in enumerate(imp_share))}.",
        "",
        "Share of fitted signal by hub pair (rows = export hub, cols = import hub):",
        "",
        "| | " + " | ".join(f"imp {j+1}" for j in range(r)) + " |",
        "|---|" + "---|" * r,
    ]
    for i in range(k):
