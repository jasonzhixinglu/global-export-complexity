...
\[
  X = R\,F\,C' + e,
\]
for $N$ countries, $k$ export hubs and $r$ import hubs, with three pieces that the stage charts draw as one column each:
\begin{itemize}\itemsep2pt
  \item $R$ ($N \times k$) are the \textbf{export loadings}: the weight of each country in each export hub.
  \item $C$ ($N \times r$) are the \textbf{import loadings}: the weight of each country in each import hub.
...
\end{itemize}
Similar to a PCA factor model, which summarizes co-movement in a vector of series as loadings times a low-dimensional vector of factors, the MFM does the same for a matrix: a low-dimensional matrix of factors with two sets of loadings, one for the rows and one for the columns.}
