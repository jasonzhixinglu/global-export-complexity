...
\begin{center}
  ...
  \begin{minipage}{\ifmobile 0.92\else 0.80\fi\linewidth}\centering
    ...
    Chart pack based on analysis of data from UN Comtrade + TDM + Trade Atlas, which I construct a bilateral monthly panel of HS6 trade flows capturing the global AI-compute supply chain (60 HS6 codes, 2017-01 to 2026-04). 
  \end{minipage}
  \vspace{22pt}
...
\sectionpage{1. The supply chain in 2024}{2024 annual trade flows from the Trade Atlas. We record all relevant HS6 codes, so it also records trade beyond solely AI compute. China and Hong Kong are counted as one trade bloc (CHK) throughout. }

\chartpage{Topography of the supply chain}{Two upstream inputs feed chip fabrication: capital equipment in orange is used to build manufacturing facilities, raw materials and wafers are used per chip. Green represents the chip fabrication, which feeds downstream assembly into AI servers. Circle size in proportion with the log of 2024 USD trade value.}{chain_topology.pdf}

\chartpage{The chain in bilateral trade flows}{Band width = 2024 dollars. Small flows are grouped into other (grey).}{overviews/supply_chain_overview_dollar_coarse_2024.pdf}

\chartpage{The chain as a graph network}{Countries as nodes, flows as edges colored by stage.}{network/supply_chain_network_2024.pdf}
