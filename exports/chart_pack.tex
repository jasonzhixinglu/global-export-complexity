...
Each stage is one square matrix of trade: $X$ has a row per exporting country
and a column per importing country, and cell $X_{ij}$ is the dollars shipped
from $i$ to $j$ in 2024. The model summarizes that matrix as
\[
  X \;\approx\; R\,F\,C' ,
\]
with three pieces, each of which the stage charts draw as one column:
\begin{itemize}\itemsep2pt
  \item $R$ (countries $\times$ 4) are the \textbf{export loadings}: how much
        each exporting country belongs to each of four export hubs. A hub is a
        group of exporters whose trade moves together across destinations.
  \item $C$ (countries $\times$ 4) are the \textbf{import loadings}, the same
        idea on the buying side.
  \item $F$ (4 $\times$ 4) are the \textbf{factors}: the dollars flowing from
        each export hub to each import hub. This is the compressed picture of
        the whole stage -- 900 country pairs summarized by 16 hub pairs.
\end{itemize}
Loadings are estimated by eigendecomposition of the pooled cross-products and
then rotated to the basis that makes them non-negative, so a loading reads as a
share rather than an arbitrary weight. Four hubs per side is the working rank;
the fit is close, with an R-squared of 0.95 to 0.99 across stages. Section 3
applies the same model month by month, letting the hubs move over time.}
