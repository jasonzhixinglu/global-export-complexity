...
\begin{center}
  ...
  \begin{minipage}{\ifmobile 0.92\else 0.80\fi\linewidth}\centering
    ...
    Chart pack based on analysis of data from UN Comtrade + TDM + Trade Atlas, from which I construct a bilateral monthly panel of HS6 trade flows capturing the global AI-compute supply chain (60 HS6 codes, 2017-01 to 2026-04).
  \end{minipage}
  \vspace{22pt}
...
\sectionpage{1. The supply chain in 2024}{2024 annual trade flows from the Trade Atlas. I record all relevant HS6 codes, so the data also covers trade beyond solely AI compute. China and Hong Kong are counted as one trade bloc (CHK) throughout.}
...
\chartpage{The chain in bilateral trade flows}{Width of band by stage = 2024 dollars. Small flows are grouped into other (grey).}{overviews/supply_chain_overview_dollar_coarse_2024.pdf}
...
\sectionpage{2. The chain in stages}{One page per stage: exporters on the left, importers on the right, with the factor model's country groupings in between. The groupings fit the 2024 flows closely (R-squared 0.95 to 0.99). All eight stages are Trade Atlas annual data.}
